...
class SuperCalendarConfiguratorLine(models.Model):
    ...
    name = fields.Many2one('ir.model', string='Model', required=True, ondelete='cascade',)
    domain = fields.Char(string='Domain',)
    configurator_id = fields.Many2one('super.calendar.configurator',
                                      string='Configurator', ondelete='cascade',
    )
    ...
    description_field_id = fields.Many2one('ir.model.fields', string='Description field',
                                           domain="[('ttype', 'in', ('char', 'text')), ('model_id', '=', name)]",
                                           ondelete='cascade',
    )
    description_code = fields.Text(string='Description field', help=("""Use '${o}' to refer to the involved object. 
    E.g.: '${o.project_id.name}'"""),)
    date_start_field_id = fields.Many2one('ir.model.fields', string='Start date field',
                                          domain="[('ttype', 'in', ('datetime', 'date')), "
                                                 "('model_id', '=', name)]",
                                          required=True, ondelete='cascade',)
    date_stop_field_id = fields.Many2one('ir.model.fields', string='End date field',
                                         domain="[('ttype', 'in', ('datetime', 'date')), "
                                                "('model_id', '=', name)]",
                                         ondelete='cascade',)
    duration_field_id = fields.Many2one('ir.model.fields', string='Duration field',
